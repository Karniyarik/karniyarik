...
List<LinkedLabel> list = estateUtil.getDistrict(searchResult);
String[] values = estateUtil.getFilterData(list, "district", request);
String data = values[0];
String value = values[1];
%>
<div class="lhead" title="Sadece belli bir ilçedeki emlak ilanlarını göster">
  <a class="sh act" href="javascript:"></a>
  İlçe </div>
<div class="llist act c1" id="district_list">
  <input type="hidden" id="district" name="district" value="<%=value%>" class="rotc"/>
  <%
  ...
  <div class="pb">
    <label>
      ...
      <span><a title="Sadece <%=linkedLabel.getLabel()%> ilçesindeki emlak ilanlarını göster" href="<%=linkedLabel.getLink()%>" <%=follow%>><%=linkedLabel.getLabel()%></a></span>
      <span class="f1" title="Bu özelliğe sahip toplam <%=linkedLabel.getCount()%> sonuç var">(<%=linkedLabel.getCount()%>)</span>
    </label>
  </div>
  <%} %> 
  <script type="text/javascript">
  setFilterValues("#district_list", "#district", [<%=data%>]);
  hookFilter("#district_list", "#district");
  </script>
</div>
<div class="lbottom c1 act">
  <a href="javascript:" onclick="cleanAndSubmit('district');">
