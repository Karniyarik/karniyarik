...
List<String> values = estateUtil.getRangeFilterVariables("area", searchResult, request);
...
<div class="lhead" title="Sadece belli bir metrekare aralığındaki emlak ilanlarını göster">
  <a class="sh act" href="javascript:"></a>
  Alanı</div>
<div class="llist act b c1">
	<div class="pb price">
		<div class="fst">
			<input name="area1" type="text" id="area1" size="7" value="<%=price1%>" class="numeric rotc fbox" />
		</div>
		<div class="scd">
			<input name="area2" type="text" id="area2" size="7" value="<%=price2%>" class="numeric rotc fbox"/>
		</div>
	</div>
	<div id="area-range" class="rangefilter"></div>
	...
	$(function() {
		$( "#area-range" ).slider({
			...
			slide: function( event, ui ) {
				resetnumeric($("#area1"),$( "#area-range" ).slider( "values", 0 ));
				resetnumeric($("#area2"),$( "#area-range" ).slider( "values", 1 ));		
			}
		});
	});
	</script>
...
<div class="lbottom c1 act">
  <a href="javascript:" onclick="cleanAndSubmit('area1,area2');">
  ...
  <input name="areamax" type="hidden" size="5" value="<%=priceMax%>" class="rotc"/>
  <input name="areamin" type="hidden" size="5" value="<%=priceMin%>" class="rotc"/>
</div>   