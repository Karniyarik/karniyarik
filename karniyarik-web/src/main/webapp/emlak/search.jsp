...
request.setAttribute(RequestWrapper.CATEGORY, CategorizerConfig.ESTATE);
...
if(searchResult.isShowNotFound()) { request.setAttribute(RequestWrapper.GA_TRCK_TYPE_VALUE, 12);}
else{request.setAttribute(RequestWrapper.GA_TRCK_TYPE_VALUE, searchResult.getSponsoredProducts().size() > 0 ? 14 : 13);}
%>  
<html xmlns:v="http://rdf.data-vocabulary.org/#" 
	...
	if(searchResult.isShowProducts() && !searchResult.isSimilar()) {
		...
		<div class="ce">
		  ...
		  if(searchResult.isShowProducts()) {
		    ...
		    %><div class="rtime c3">
		    	...
		    	<%=NumberFormat.getInstance(Locale.ENGLISH).format(searchResult.getTotalHits())%> emlak ilanı bulundu
		    </div><%
		    }
		...
		%></div>
		  <div class="cl"></div>
		  <div class="l">    
		    <div class="filter"><%if(searchResult.isShowToolbar()){ 
		      ...
		      <div class="si" title="Emlak ilanlarını arama sorgusu ile ilişkisinin benzerliğine göre sırala">
		      ...
		      <div class="si" title="Emlak ilanlarını fiyata göre ucuzdan/pahalıya ya da pahalıdan/ucuza göre sırala">
			...
			%></div>
		    <div class="cl"></div>
		    <div class="lc <%=searchResult.isShowProducts()? "" : "full"%> estate">
		      ...
		      %><div class="spon">
		          ...
		          %><img src="<%=imgUrl%>" alt="Ürün resmi" property="v:photo" content="<%=imgUrl%>"/><%
		          } 
		          %></div>
		          <div class="prinf">
		            <div class="spcl">Özel Ürün</div>
		            <div class="h">
		            	<a rel="nofollow" target="_blank" href="<%=sponsored.getLink()%>" title="Emlak ilanına git" 
				 		onclick="productClicked('<%=sponsored.getLink()%>', '<%=sponsored.getSourceName()%>', '<%=sponsored.getProductName()%>', 9)">
		            ...
		            <div class="sr">
		              ...
		              &nbsp;
					  <a rel="nofollow v:url" title="Benzer Emlak İlanları" href="<%=sponsored.getShareLink()%>" class="c2">Benzer İlanlar</a>
					  <img width="14" height="13" class="sprite more-btn" src="<%=request.getContextPath()%>/images/transparent.gif" alt="Benzer Emlak İlanları"/>
		            </div>            
		          </div>
		          <div class="prpi">
		            <div class="pprice">
		            ...
		            <div>
			            <a target="_blank" href="<%=sponsored.getLink()%>" rel="nofollow" title="Emlak ilanına git"
			            onclick="productClicked('<%=sponsored.getLink()%>', '<%=sponsored.getSourceName()%>', '<%=sponsored.getProductName()%>', 9)">
			            	<img class="sprite go-site-btn" src="<%=request.getContextPath()%>/images/transparent.gif" width="119" height="23" alt="Emlak ilanına git"/>
			            </a>              
		            </div>
		          </div>
		        </div><%
			...
			%><div class="prods">
				...
				%><div class="cl"></div>	
		        <div class="others"><h2>Benzer Emlak İlanları</h2></div><%
		        }
		        %><div class="prod <%=product.isSingleShared() ? "snglshrd" : ""%>" typeof="v:Product">
		          <div class="pri"><%
		          String imgUrl = ImageServlet.getImageRszUrl(request, product.getImageURL(), product.getImageName(), 140, 150);
		          if(searchResult.isShowImages() == 1){%>
		          <img src="<%=imgUrl%>" alt="Emlak İlanı resmi" property="v:photo" content="<%=imgUrl%>"/><%
		          } 
		          %></div>
		          <div class="prinf">
		            <div class="h">
		            	<a target="_blank" href="<%=product.getLink()%>" rel="nofollow" title="Emlak ilanına git" 
				 		onclick="productClicked('<%=product.getLink()%>', '<%=product.getSourceName()%>', '<%=product.getProductName()%>', 8)">
		            ...
		            <div class="ad add">
		            	<span><%=product.getProperty("address")%>, <%=product.getProperty("district")%>, <%=product.getProperty("city")%></span> &nbsp;
		             </div>
		            <div class="ad" style="height:40px;"><% 
		            if (!StringUtils.isEmpty(product.getProperty("area"))) { 
		            %><div class="drow"><span class="c1">Alanı: </span><%=product.getProperty("area")%>
		            </div><% 
		            } 
		            if (!StringUtils.isEmpty(product.getProperty("warmingtype"))) { 
		            %><div class="drow"><span class="c1">Isınma: </span><%=product.getProperty("warmingtype") 
		            %></div><% 
		            } 
		            if (!StringUtils.isEmpty(product.getProperty("roomcount"))) { 
		            %><div class="drow"><span class="c1">Oda Sayısı: </span><%=product.getProperty("roomcount") %> + <%=product.getProperty("salooncount") %>
		            </div><% 
		            } 
		            if (!StringUtils.isEmpty(product.getProperty("floorcount"))) { 
		            %><div class="drow"><span class="c1">Katı: </span><%=product.getProperty("floorcount") %></div><% 
		            } 
		            if (!StringUtils.isEmpty(product.getProperty("buildingage"))) { 
		            %><div class="drow"><span class="c1">Bina Yaşı: </span><%=product.getProperty("buildingage") %></div><% 
		            } 
		            if (!StringUtils.isEmpty(product.getProperty("buildingtype"))) { 
		            %><div class="drow"><span class="c1">Emlak Tipi: </span><%=product.getProperty("buildingtype") %></div><% 
		            } 
		            %></div>
		            ...
		            <div class="sr">
					  ...
					  <a rel="nofollow v:url" title="Benzer Emlak İlanları" href="<%=product.getShareLink()%>" class="c2">Benzer İlanlar</a>
		              <img width="14" height="13" class="sprite more-btn" src="<%=request.getContextPath()%>/images/transparent.gif" alt="Benzer Emlak İlanları"/>
					</div>
		          </div>
		          <div class="prpi">
		    ...
		    %><jsp:include page="activefilters.jsp"></jsp:include>
		      ...
		      <jsp:include page="filterdistrict.jsp"></jsp:include>
		      <jsp:include page="filterbuildingtype.jsp"></jsp:include>
		      <jsp:include page="filterarea.jsp"></jsp:include>
		      <jsp:include page="filterwarming.jsp"></jsp:include>
		      <jsp:include page="filterroomcount.jsp"></jsp:include>
		      <jsp:include page="filterfloorcount.jsp"></jsp:include><%
		      }
			if(searchResult.isSimilar() && !searchResult.isShowURLNotFoundError() && !searchResult.isShowEmptySearchError()) {
		      %><div class="lads b">
				...
				/* estateSimilarTopLeft */
				google_ad_slot = "3364888810";
				google_ad_width = 160;
