...
List<LinkedLabel> list = estateUtil.getBuildingType(searchResult);
String[] values = estateUtil.getFilterData(list, "buildingtype", request);
String data = values[0];
String value = values[1];%>
<div class="lhead" title="Sadece belli bir emlak tipindeki emlak ilanlarını göster">
  <a class="sh act" href="javascript:"></a>
  Emlak Tipi </div>
<div class="llist act c1" id="buildingtype_list">
  <input type="hidden" id="buildingtype" name="buildingtype" value="<%=value%>" class="rotc"/><%
  ...
  <div class="pb">
    <label>
      ...
      <span><a title="Sadece <%=linkedLabel.getLabel()%> emlak tipindeki ilanları göster" href="<%=linkedLabel.getLink()%>" <%=follow%>><%=linkedLabel.getLabel()%></a></span>
      <span class="f1" title="Bu özelliğe sahip toplam <%=linkedLabel.getCount()%> sonuç var">(<%=linkedLabel.getCount()%>)</span>
    </label>
  </div><%
  } 
  %><script type="text/javascript">
  	setFilterValues("#buildingtype_list", "#buildingtype", [<%=data%>]);
  	hookFilter("#buildingtype_list", "#buildingtype");
  </script>
</div>
<div class="lbottom c1 act">
  <a href="javascript:" onclick="cleanAndSubmit('buildingtype');">
