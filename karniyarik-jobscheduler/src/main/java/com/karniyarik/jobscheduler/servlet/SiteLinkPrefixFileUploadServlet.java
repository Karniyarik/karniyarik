...
public class SiteLinkPrefixFileUploadServlet extends HttpServlet
{
	@Override
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException
	{
		...
		if(items.size()>0)
		{
			...
			String endpoint = deploymentConfig.getMasterWebUrl() + "/rest/event/updatesiteprefix";
			JerseyUtil.sendJSONPut(endpoint, content);
			
			result = true;
		}
		
		resp.sendRedirect(req.getContextPath() + "/uploadsitelinkprefix.jsp?result=" + (result ? "success" : "fail") + "&files=" + fileStr);
	}
