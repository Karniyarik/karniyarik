...
public class CitiesInTurkeyRegistry {
	
	private Map<String, AlternateNamesHolder> codeMap = new HashMap<String, AlternateNamesHolder>();
	private List<String> cities = new ArrayList<String>();
	
	private static CitiesInTurkeyRegistry instance = new CitiesInTurkeyRegistry();
	
	private CitiesInTurkeyRegistry() {
		try {
			InputStream stream = StreamUtil.getStream("turkey_cities.txt");
			...
			throw new RuntimeException("Cannot read cities in Turkey", e);
		}
	}
	
	public static CitiesInTurkeyRegistry getInstance() {
		return instance;
	}
	
	public String getCity(String city)
	{
		String tmp = normalize(city);
		AlternateNamesHolder cityHolder = codeMap.get(tmp);
		if(cityHolder != null)
		{
			return cityHolder.getName();
		}
		else
		{
